...
-----END RSA PRIVATE KEY-----`))
	...
	if err != nil {
		t.Fatalf("Unexpected error while parsing private key: %v", err)
	}
	if err := pk.Validate(); err != nil {
		t.Fatalf("Unexpected error while validating test private key: %v", err)
	}
	...
	t.Run("verify token with RS256 signature", func(t *testing.T) {
		token := jwt.JWT{
			Head: &jwt.JWTHead{
				Alg: "RS256",
				Typ: "JWT",
		...
		pubGetter := func(_ string) (*rsa.PublicKey, error) { return &pk.PublicKey, nil }

		if err := token.Verify(pubGetter); err != nil {
